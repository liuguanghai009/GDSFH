function Complementary_Whitening

Tests = ["Oxford_5K库","Paris_6K库","Oxford_105K库","Paris_106K库","Holidays_upright库","UKB库","ROxford5k库","RParis6k库","GHIM_10K库","Corel_10K库"];
Trains = ["Oxford_5K库","Paris_6K库","Oxford_105K库","Paris_106K库","Holidays_upright库","UKB库","ROxford5k库","RParis6k库","GHIM_10K库","Corel_10K库"];
...
fileXTrain = dir("F:/标准图像库/" + dataXTrain + "/Matlab提取的特征/DOFH/负例/" + "*.txt");

%
[file_num, ~] = size(fileXTest);

[file_count, ~] = size(fileXTrain);
%
...
parfor i=1:file_count
    ...
    negative_deepCrow(i,:) = importdata("F:/标准图像库/" + dataXTrain{1}  + "/Matlab提取的特征/DOFH/负例/" + fileXTrain(i).name);
...
X = whitening_V2(negative_deepCrow,positive_deepCrow,dim);
