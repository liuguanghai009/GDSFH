classdef ClassAttention < nnet.layer.Layer
    ...
    methods
        ...
        function layer = ClassAttention(Name,dim,num_heads,nc)
            ...
            layer.Description = 'ClassAttention';
        ...
        function Z = predict(layer,X)    
            ...
            q = layer.q.predict(X(1,:,:)); % (N, C, B) -> (N, C, B)

            q = stripdims(q);

            q = reshape(permute(q,[2,1,3]),C/layer.numheads,layer.numheads,[],B); % (C/nh, nh , N, B)
            ...
            k = stripdims(k);
            q = stripdims(q);
            attn = pagemtimes(q*layer.scale,permute(k,[2,1,3,4,5])) ;  % (N, C/nh, nh, B) (N_p, C_p/nh, nh, B) -> (N, N_p, nh, B)
